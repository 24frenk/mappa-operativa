...
function checkPIN() {
    ...
    if (input === ADMIN_PIN) {
        ...
        loadState();        // carica eventuale stato locale
        loadFromGitHub();   // 🔥 carica data.json da GitHub
    } else {
...
async function loadFromGitHub() {
    ...
    try {
        ...
        // salviamo lo stato in localStorage
        localStorage.setItem("mapState_full", JSON.stringify(json));

        // ricostruiamo la mappa
        loadState();
...
window.onload = () => {
    // quando l'admin inserisce il PIN, carichiamo data.json da GitHub
};
